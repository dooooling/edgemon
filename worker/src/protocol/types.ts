...
export function validateHelloPayload(data: HelloPayload): boolean {
  ...
  const physical = (data.resources as { cpu_physical_cores?: unknown }).cpu_physical_cores;
  if (physical !== undefined && physical !== null) {
    if (typeof physical !== 'number' || !Number.isFinite(physical) || physical < 0) return false;
  }
